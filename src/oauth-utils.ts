...
.client-info{border:1px solid #e5e7eb;border-radius:6px;padding:1rem 1rem .5rem;margin-bottom:1.5rem}
.client-name{font-weight:600;font-size:1.2rem;margin:0 0 .5rem}
.client-detail{display:flex;margin-bottom:.5rem;align-items:baseline}
.detail-label{font-weight:500;min-width:120px}
.detail-value{font-family:SF Mono,Menlo,Monaco,Consolas,monospace;word-break:break-all}
...
${redirectUris.length > 0 ? `<div class="client-detail"><div class="detail-label">Redirect URIs:</div><div class="detail-value">${redirectUris.map((uri) => `<div>${uri}</div>`).join("")}</div></div>` : ""}
